...
describe('Deployment Router', () => {
  ...
  describe('GET /deployment', () => {
    test('should return 200 with data', async () => {
      ...
      jest.spyOn(mockGetDeploymentUseCase, 'execute').mockImplementation(async () => await Promise.resolve(ExpectedData))
    ...
    test('GET /deployment returns 500 on use case error', async () => {
      jest.spyOn(mockGetDeploymentUseCase, 'execute').mockImplementation(async () => await Promise.reject(Error('Error fetching data')))
      const response = await request(server).get('/deployment')
