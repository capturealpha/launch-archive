...
export class UrlService {
  ...
  static getStartedWallet = (section?: string) =>
    `/get-started/wallet${appendSearchParams({ section })}`;

  static sdlBuilder = (id?: string) =>
    `/sdl-builder${appendSearchParams({ id })}`;
  ...
  static block = (height: number) =>
    `/blocks/${height}${appendSearchParams({
      network: getSelectedNetworkQueryParam(),
    })}`;
  static transactions = () => `/transactions`;
  static transaction = (hash: string) =>
    `/transactions/${hash}${appendSearchParams({
      network: getSelectedNetworkQueryParam(),
    })}`;
  static address = (address: string) =>
    `/addresses/${address}${appendSearchParams({
      network: getSelectedNetworkQueryParam(),
    })}`;
  static addressTransactions = (address: string) =>
    `/addresses/${address}/transactions`;
  static addressDeployments = (address: string) =>
    `/addresses/${address}/deployments`;
  static validators = () => '/validators';
  static validator = (address: string) =>
    `/validators/${address}${appendSearchParams({
      network: getSelectedNetworkQueryParam(),
    })}`;
  ...
  static deploymentDetails = (dseq: string, tab?: string, logsMode?: string) =>
    `/deployments/${dseq}${appendSearchParams({ tab, logsMode })}`;
  static publicDeploymentDetails = (owner: string, dseq: string) =>
    `/deployment/${owner}/${dseq}${appendSearchParams({
      network: getSelectedNetworkQueryParam(),
    })}`;
  static templates = (category?: string, search?: string) =>
    `/templates${appendSearchParams({ category, search })}`;
