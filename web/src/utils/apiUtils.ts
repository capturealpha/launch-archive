...
export class ApiUrlService {
  ...
  static leaseList(apiEndpoint: string, address: string, dseq: string) {
    return `${apiEndpoint}/akash/market/${networkVersion}/leases/list?filters.owner=${address}${
  ...
  static addressDeployments(
    ...
    return `${BASE_API_URL}/addresses/${address}/deployments/${skip}/${limit}${appendSearchParams(
      { reverseSorting, ...filters }
    )}`;
  }
...
export async function loadWithPagination(
  baseUrl: string,
  dataKey: string,
  limit: number
) {
  ...
  do {
    ...
    let queryUrl = `${baseUrl}${
      _hasQueryParam ? '&' : '?'
    }pagination.limit=${limit}&pagination.count_total=true`;
    if (nextKey) {
