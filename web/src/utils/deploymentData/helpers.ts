...
export function parseSizeStr(str: string) {
  try {
    const suffix = Object.keys(specSuffixes).find((s) =>
      str.toLowerCase().endsWith(s.toLowerCase())
    );
...
function isString(value: unknown): value is string {
  return (
    typeof value === 'object' && value !== null && value.constructor === String
  );
}

export function getSdl(yamlJson: string | v2Sdl, networkType: NetworkType) {
  return isString(yamlJson)
    ? SDL.fromString(yamlJson, networkType)
    : new SDL(yamlJson, networkType);
}

export function DeploymentGroups(
