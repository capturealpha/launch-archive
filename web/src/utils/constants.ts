...
export const validatorAddress =
  'akashvaloper14mt78hz73d9tdwpdvkd59ne9509kxw8yj7qy8f';
...
function getApiTestnetUrl() {
  ...
  if (typeof window === 'undefined') return 'http://localhost:3080';
  if (window.location?.hostname === 'deploy.cloudmos.io')
    return productionTestnetApiUrl;
  return 'http://localhost:3080';
}
...
function getApiSandboxUrl() {
  ...
  if (typeof window === 'undefined') return 'http://localhost:3080';
  if (window.location?.hostname === 'deploy.cloudmos.io')
    return productionSandboxApiUrl;
  return 'http://localhost:3080';
}
...
function getProviderProxyHttpUrl() {
  ...
  if (window.location?.hostname === 'deploybeta.cloudmos.io')
    return 'https://deployproxybeta.cloudmos.io';
  if (window.location?.hostname === 'deploy.cloudmos.io')
...
function getProviderProxyWsUrl() {
  ...
  if (window.location?.hostname === 'deploybeta.cloudmos.io')
    return 'wss://deployproxybeta.cloudmos.io';
  if (window.location?.hostname === 'deploy.cloudmos.io')
