...
import { mainnetId, mainnetNodes } from '../../utils/constants';
import { useLocalStorage } from '../../hooks/useLocalStorage';
import { migrateLocalStorage } from '../../utils/localStorage';
import { initAppTypes } from '../../utils/init';
...
export const SettingsProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initiateSettings = async () => {
      ...
      const _selectedNetworkId =
        localStorage.getItem('selectedNetworkId') || mainnetId;
      ...
      const hasSettings =
        ...
        nodes.find((x) => x.id === settings.selectedNode.id);
  ...
  const clearQueries = (prevSettings, newSettings) => {
    if (
      prevSettings.apiEndpoint !== newSettings.apiEndpoint ||
      (prevSettings.isCustomNode && !newSettings.isCustomNode)
    ) {
  ...
  const refreshNodeStatuses = useCallback(
    async (settingsOverride?) => {
      ...
      let _customNode = settingsOverride
        ? settingsOverride.customNode
        : customNode;
      let _isCustomNode = settingsOverride
        ? settingsOverride.isCustomNode
        : isCustomNode;
      let _apiEndpoint = settingsOverride
        ? settingsOverride.apiEndpoint
        : apiEndpoint;
      ...
      setSettings((prevSettings) => {
        const selectedNode = _nodes.find(
          (node) => node.id === prevSettings.selectedNode.id
        );
