...
export const KeplrWalletProvider = ({ children }) => {
  ...
  async function loadWallet(): Promise<void> {
    ...
    const storageWallets = JSON.parse(
      localStorage.getItem(`${selectedNetworkId}/wallets`)
    );

    const currentWallets = storageWallets ? [...storageWallets] : [];
    const newWallets =
      currentWallets.findIndex((x) => x.address === address) === -1
        ? [...currentWallets].concat({ name: wallet?.name, address })
        : [...currentWallets];
  ...
  async function signAndBroadcastTx(msgs: EncodeObject[]): Promise<boolean> {
    ...
    try {
      ...
      if (
        err.message.includes('was submitted but was not yet found on the chain')
      ) {
        errorMsg = 'Transaction timeout';
      } else {
        try {
          const reg =
            /Broadcasting transaction failed with code (.+?) \(codespace: (.+?)\)/i;
          const match = err.message.match(reg);
          const log = err.message.substring(
            err.message.indexOf('Log'),
            err.message.length
          );
  ...
  async function refreshBalances(
    address?: string
  ): Promise<{ uakt: number; usdc: number }> {
